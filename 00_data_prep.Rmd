...
extract_nyu <- function(con, tblname, yr, out_path = out) {
  ...
  nyu_pluto <- tryCatch(st_read(con, query = paste0(
    ...
    select(c("assessed_value", "cd", "bldgarea", "landuse",
             "owername"), matches("area"),
           matches("address"), "bbl"),
  error = function(cond) {
...
nyu_pluto_app <- bind_rows(nyu_pluto_full, pluto2008, rpad_filt)
nyu_pluto_app
...
nyu_pluto_app %>%
  ...
  st_transform(2263) %>% 
  select(-"geom_point") %>% 
  select(matches("assess"), starts_with("cd"),
         matches("land"), "ownername", matches("area"),
           matches("address"), "bbl") %>% 
