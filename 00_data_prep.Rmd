...
tbl(con, "rpad_raw") %>% 
  filter(year4 == 1999) %>% 
  left_join(tbl(con, "rpad_condo_xwalk"), by = "bbl_condo") %>% 
  filter(bbl_condo != bbl_unit) %>% 
  select(matches("bbl"))
```
```{r}
## checking 330 east 30th street, seems incorrect
tbl(con, "rpad_raw") %>% 
  filter(bbl == "1009367501") %>% 
  arrange(year4) %>% 
  select(c("owner"), everything())

# seems like discrepancy of condo vs. unit
# looks like nyu owns a condo, but this condo won't show up
# in pluto bc NYU doesn't own the whole building
rpad_final %>% 
  filter(block == 936) %>% 
  arrange(year) %>% 
  select(c("ownername"), everything()) %>% 
  select(c("bbl", "bbl_condo"))
```
```{r}
rpad_allyrs <- dbGetQuery(con, "SELECT * FROM rpad_raw
                            WHERE owner ~* 
                          '^(NYU)$|(\\sNYU,?\\s)|(NEW YORK UNIV.+)|^(NYU,?\\s)|(NY UNIV)'")
rpad_allyrs %>% 
  arrange(year4) %>% 
  tablist_qc(year4)
...
rpad_allyrs %>% 
  ...
  rename(bldgarea = gr_sqft,
         ...
         ownername = owner,
         bldgclass = bldgcl) %>% 
  writeRDS(path(out, "rpad_allyrs.RDS"))
```
```{r}
rpad_allyrs_pts <- st_read(con, query = "SELECT * FROM rpad_raw_with_pts
                            WHERE owner ~* 
                          '^(NYU)$|(\\sNYU,?\\s)|(NEW YORK UNIV.+)|^(NYU,?\\s)|(NY UNIV)'")
...
rpad_allyrs_pts %>% 
  ...
  rename(bldgarea = gr_sqft,
         ...
         ownername = owner,
         bldgclass = bldgcl) %>% 
  st_write(rpad_allyrs_pts,
           dsn = path(out, "rpad_allyrs_pts.geojson"))
```
...
rpad_check <- st_read(con, query = "SELECT r.*, m.geom_poly FROM rpad_raw as r
