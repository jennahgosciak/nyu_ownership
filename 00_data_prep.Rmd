...
```{r}
tbl(con, "rpad_raw") %>% 
  filter(bbl == '1009620001' | (str_detect(str_name, "EAST 30") & hnum_lo == "00433")) %>% 
  select(c("year4", "owner"), everything()) %>% 
  arrange(year4)

rpad %>% 
  filter(str_detect(str_name, "MAC DOUGAL")) %>% 
  select(c("year4", "str_name", "bbl", "owner"), everything()) %>% 
  as_tibble()
```
...
rpad_final <- rpad_lt2002 %>% 
  as_tibble() %>% 
  bind_rows(as_tibble(rpad_unmerged)) %>% 
  unique()  %>% 
  rename(bldgarea = gr_sqft,
         year = year4,
         ownername = owner,
