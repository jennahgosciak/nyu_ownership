...
nyu_21 %>% 
  ...
  geom_sf(data = parks,
            fill = "#777777", color = NA) +
  geom_sf(fill = "#9057FF", color = NA) +
  geom_sf(data = cds, color = "white", fill = NA, size = 0.1) +
  theme_void() +
  theme(plot.background = element_rect(fill = '#414141'))

ggsave(path(fig_path, "nyu_21_map.pdf"), height = 15, width = 15)
ggsave(path(fig_path, "nyu_21_map.svg"), height = 15, width = 15)
```


```{r}
# create a dataframe of the properties
nyu_df <- nyu_pluto_app %>% 
  select(-any_of(drop_vars)) %>% 
...
nyu_assess_value <- nyu_df %>%
  select("bbl", "cd", "year", matches("assess"),
         "bldgarea", "geom_point", "floorarea",
         "ownername", "address", "borough") %>% 
  mutate(borough = str_sub(cd, 1, 1),
           cd = as.numeric(cd)) %>% 
  ...
  mutate(assessed_value_orig = max_noinf(assesstotl, 
                                   ...
                                   assesstot)) %>% 
  ungroup() %>% 
  mutate(bldgarea = if_else(is.na(bldgarea), 
                            floorarea, as.double(bldgarea))) %>%
